...
\usepackage[landscape,paper=a4paper,left=2mm,right=2mm,top=5mm,bottom=5mm,footskip=2mm]{geometry}
...
For $t \in [n]$ find $z = \argmin_j||\mu_j-\bm x_t||_2$, set $\mu_c \leftarrow \mu_c + \eta_t(\bm x_t - \mu_c)$. For local optimum: $\sum_t \eta_t = \infty \wedge \sum_t \eta_t^2 < \infty$ suffices, e.g.\ $\eta_t = \frac{c}{t}$, $c \in \R$.
...
If $F(S)$ subm.\ on $V$, then $F(S \cap W), F(S \cup W), F(V \setminus S)$ subm.\ where $W \subseteq V$.
