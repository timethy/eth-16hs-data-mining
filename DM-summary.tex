...
\usepackage[extreme,margins=tight]{savetrees}
...
\paragraph{Relevant version space}
...
\paragraph{Weighted rep. $C$} of large $D$: $L_k(\mu;C) = \sum_{(w,\bm x)\in C}w\cdot\min_j||\mu_j - \bm x||_2^2$.

\paragraph{$(k,\eps)$-coreset $C$ of $D$} iff $\forall\mu: (1-\eps)L_k(\mu;D) \leq L_k(\mu;C) \leq (1+\eps)L_k(\mu;D)$.
