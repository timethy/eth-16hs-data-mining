...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item $v$ ist \emph{wirbelfrei}: $\rot(\grad f) = \vec{0}$.
 \item Kurvenintegrale über $v$ sind nur abhängig von Anfangs- und Endpunkt.
 \item Kurvenintegrale über $v$ mit Anfangspunkt $=$ Endpunkt sind $0$.
\end{enumerate}
...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item Gilt $det < 0: k$ ist Sattelpunkt von $f$.
 \item Gilt $det > 0 \wedge (H_f)_{1,1} > 0: k$ ist Minimum von $f$.
 \item Gilt $det > 0 \wedge (H_f)_{1,1} < 0: k$ ist Maximum von $f$.
 \item Gilt $det = 0$, keine Aussage, weiteres Vorgehen nötig.
\end{enumerate}

\subsection{Integration im $\R^m$}
