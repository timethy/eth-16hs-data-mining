...
\fancyhead[R]{Tim Taubner, B.\ Sc.\ ETH Informatik, Legi-Nr.: 12-937-546}
...
Konvergiert $\sum_{n=1}^\infty a_n$, so muss $\lim_{n \to \infty} a_n = 0$ gelten.
Ist $(a_n)$ also keine Nullfolge, konvergiert auch obige Reihe nicht!
...
\paragraph{Majoranten- und Minorantenkriterium}
...
\]
und es konvergiert die Reihe der Produkte absolut, unabhängig von der Summationsreihenfolge.


\subsubsection{Kriterien}
	Die nachfolgenden Kriterien sagen nur aus, ob die Reihen konvergiert oder nicht. Sie sagen \underline{nicht} aus, gegen was sie konvergieren.
...
\subsection{Potenzreihe}
	...
	$x_0$ ist der Entwicklungspunkt der Potenzreihe und $(a_n)_{n \in \N}$ eine beliebige Folge.

	\subsubsection{Konvergenzradius}
		Die Berechnung des Konvergenzradius ist für solche Reihen einfacher, da der Faktor $(x - x_0)$ nicht analysiert werden muss. Entsprechend gilt für den Konvergenzradius $r$:
		$r = \frac{1}{\limsup_{n\to\infty} \sqrt[n]{|a_n|}}$ (Wurzelkriterium) bzw.
		$r = \lim_{n\to\infty} \left | \frac{a_n}{a_{n+1}} \right |$ (Quotientenkriterium, nur wenn ab einem bestimmten Index alle $a_n$ von 0 verschieden sind und der Limes existiert, nicht immer anwendbar).\\
		Konvergiert für alle $|z| < r$, divergiert für alle $|z| > r$
		
	\subsubsection{Randpunkte}
		\begin{itemize}
			\item Für das Konvergenzverhalten der Randpunkte bedarf es weiterer Untersuchungen.
			\item Dabei müssen die Randpunkte einzeln in die vorgegebene Reihe eingesetzt werden. Wenn die Reihe divergiert, ist der Randpunkt ebenfalls divergent und umgekehrt.
		\end{itemize}

\subsubsection{Rechenregeln}

\subsubsection{Potenzreihen}
\paragraph{Konvergenzradius}
