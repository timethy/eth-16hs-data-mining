...
Eine Folge $(a_n)_{n\in\N}$ heißt \emph{Cauchy-Folge}, falls
 \[ \forall \varepsilon > 0: \exists n_0 \in\N: \forall n,l \geq n_0: |a_n - a_l| < \varepsilon \]
...
\paragraph{Cauchy-Kriterium}
Die Folge $a_i = \tfrac{1}{i}$ ist eine Cauchy-Folge. Man kann nämlich zu einem beliebig vorgegebenen $\varepsilon>0$ ein $n_0$ so wählen, dass $n_0 > \tfrac{1}{\varepsilon}$ erfüllt ist. Sind nun $n\geq m>n_0$ beliebig gewählt, dann gilt

\[ | a_m - a_n | = \left|\frac{1}{m} - \frac{1}{n}\right| = \left|\frac{n-m}{mn}\right| \leq \frac{n}{mn} = \frac{1}{m} < \frac{1}{n_0} < \varepsilon\].
